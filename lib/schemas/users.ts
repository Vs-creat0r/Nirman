...
export const usersSchema = z.object({
  ...
  username: z.string().min(3, "User ID is required").max(50, "User ID is too long").optional(),
  email: z.string().max(180, "Email is too long").regex(/^[^@\s]+@[^@\s]+\.[^@\s]+$/, "Email is not in the expected format").optional(),
